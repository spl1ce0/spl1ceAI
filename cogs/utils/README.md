...
| :--- | :--- | :--- |
| **`guild_settings`** | Server-specific configuration, tier flags, custom prompts, and BYOK keys. | `guild_id`, `prefix`, `cbc`, `log_channel`, `llm_primary`, `llm_backup1`, `llm_backup2`, `llm_backup3`, `llm_timeout`, `show_model`, `reply_ping`, `is_premium`, `custom_prompt`, `byok_gemini_key`, `byok_xai_key`, `byok_openai_key`, `byok_anthropic_key` |
